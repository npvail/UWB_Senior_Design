"""
Configuration constants for UWB tracking system
"""

# Color definitions (used for data, not display)
RED = [255, 0, 0]
BLACK = [255, 255, 255]  # Changed to white for better contrast if needed

# UWB System Configuration
ANC_COUNT = 3
TAG_COUNT = 1

# Anchor positions (in cm)
A0X, A0Y = 0, 0
A1X, A1Y = 603 , 0
A2X, A2Y = 0, 651

# Anchor Z-heights (in cm) - height off the floor
A0Z = 200.0  # Default height for anchor 0
A1Z = 200.0  # Default height for anchor 1
A2Z = 200.0  # Default height for anchor 2

# Tag height (in cm) - approximate height of the tracked object
TAG_HEIGHT = 100.0  # Default height for tag

# Map dimensions (in cm)
MAP_WIDTH_CM = 2000
MAP_HEIGHT_CM = 2000

# Serial port configuration
SERIAL_BAUDRATE = 115200
SERIAL_SLEEP_TIME = 0.01  # Delay in seconds between serial reads

# Flask configuration
FLASK_HOST = '0.0.0.0'
FLASK_PORT = 2406

