let mapWidthCm = 1000;
let mapHeightCm = 800;
let isAdmin = false; // Track if user is admin (host machine)
...
function resizeCanvas() {
    ...
    // Calculate the aspect ratio of the physical map (must be maintained)
    const mapAspectRatio = mapWidthCm / mapHeightCm;
    
    // Calculate available space aspect ratio
    const containerAspectRatio = containerWidth / containerHeight;
    
    // UNIFORM SCALING: Calculate scale factor to maintain 1:1 physical dimensions
    // We need to ensure 1cm in width = 1cm in height on screen
    // Calculate scale based on which dimension is more limiting
    let scaleX = containerWidth / mapWidthCm;  // pixels per cm horizontally
    let scaleY = containerHeight / mapHeightCm; // pixels per cm vertically
    
    // Use the SMALLER scale to ensure we fit and maintain uniform scaling
    // This ensures 1 meter in any direction = same pixel size
    const uniformScale = Math.min(scaleX, scaleY);
    
    // Calculate final dimensions using uniform scale
    const finalWidth = mapWidthCm * uniformScale;
    const finalHeight = mapHeightCm * uniformScale;
    
    // Set container and canvas dimensions - MUST maintain exact aspect ratio
    ...
    // Ensure canvas CSS size matches internal size (prevents browser stretching)
    canvas.style.width = finalWidth + 'px';
    canvas.style.height = finalHeight + 'px';
    
    // Store scale for drawing calculations
    window.pixelsPerCm = uniformScale;
}

function draw(data) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    // Use uniform scale - ensures 1cm = same pixels in all directions
    const pixelsPerCm = window.pixelsPerCm || (canvas.width / mapWidthCm);
...
function updateSidebar(data) {
    ...
    // Update anchor Z-heights - only create once, then update values
    const anchorZHeightsDiv = document.getElementById('anchor-z-heights');
    if (anchorZHeightsDiv && data.anchor_z_heights) {
        // Only create inputs if they don't exist yet (similar to anchor positions)
        if (anchorZHeightsDiv.querySelectorAll('.anchor-input').length === 0) {
            // Create inputs for each anchor
            data.anchors.forEach((anchor, index) => {
                if (index < data.anchor_z_heights.length) {
                    const div = document.createElement('div');
                    div.className = 'anchor-input';
                    div.innerHTML = `<label for="${anchor.name}-z">${anchor.name} Z:</label>
                        <input type="number" id="${anchor.name}-z" value="${data.anchor_z_heights[index]}" step="0.1" min="0">`;
                    anchorZHeightsDiv.appendChild(div);
                }
            });
        } else {
            // Update existing input values only if user is not editing them
            data.anchors.forEach((anchor, index) => {
                if (index < data.anchor_z_heights.length) {
                    const input = document.getElementById(`${anchor.name}-z`);
                    if (input && document.activeElement !== input) {
                        input.value = data.anchor_z_heights[index];
                    }
                }
            });
        }
    ...
    // Update tag height
    if (data.tag_height !== undefined) {
        const tagHeightInput = document.getElementById('tag-height-input');
        if (tagHeightInput && document.activeElement !== tagHeightInput) {
            tagHeightInput.value = data.tag_height;
        }
    }
    ...
    data.tags.forEach(tag => {
        ...
        // Build ranges HTML (bigger, displayed first)
        let rangesHTML = '';
        if (tag.ranges && tag.ranges.length > 0) {
            rangesHTML = '<div class="tag-ranges">';
            tag.ranges.forEach((range, index) => {
                if (index < data.anchors.length) { // Ensure we don't go out of bounds
                    // Convert cm to inches (1 cm = 0.393701 inches)
                    const rangeInches = (range * 0.393701).toFixed(2);
                    rangesHTML += `<div class="range-item">${data.anchors[index].name}: ${range} cm (${rangeInches} in)</div>`;
                }
            });
            rangesHTML += '</div>';
        } else {
            rangesHTML = '<div class="tag-ranges"><div class="range-item">No range data</div></div>';
        }
        
        // Tag position (smaller, displayed second)
        const tagPositionHTML = `<div class="tag-position">${tag.name}: (${tag.x}, ${tag.y}) cm</div>`;
        
        // Order: ranges first (bigger), then position (smaller)
        tagBox.innerHTML = rangesHTML + tagPositionHTML;
...
function updateZoneList(zones) {
    ...
    document.querySelectorAll('.zone-item-delete').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            if (!isAdmin) {
                alert('Unauthorized: Only the host machine can edit settings.');
                return;
            }
            ...
            try {
                ...
                if (response.status === 403) {
                    alert(result.message || 'Unauthorized: Only the host machine can edit settings.');
                    return;
                }
...
document.getElementById('save-anchors-btn').addEventListener('click', async () => {
    ...
    const updatedAnchors = Array.from(document.querySelectorAll('#anchor-controls .anchor-input')).map(div => ({
...
document.getElementById('save-heights-btn').addEventListener('click', async () => {
    if (!isAdmin) {
        alert('Unauthorized: Only the host machine can edit settings.');
        return;
    }
    const anchorZHeights = [];
    const anchorZHeightsDiv = document.getElementById('anchor-z-heights');
    if (anchorZHeightsDiv) {
        // Get all anchor Z-height inputs in order
        const anchorInputs = anchorZHeightsDiv.querySelectorAll('input[type="number"]');
        anchorInputs.forEach(input => {
            const value = parseFloat(input.value);
            if (!isNaN(value) && value >= 0) {
                anchorZHeights.push(value);
            } else {
                anchorZHeights.push(0);
            }
        });
    }
    
    // Ensure we have the correct number of heights
    while (anchorZHeights.length < 3) {
        anchorZHeights.push(0);
    }
    
    const tagHeightInput = document.getElementById('tag-height-input');
    const tagHeight = tagHeightInput ? (parseFloat(tagHeightInput.value) || 0) : 0;
    
    try {
        const response = await fetch('/update_heights', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                anchor_z_heights: anchorZHeights,
        ...
        document.getElementById('status-message').textContent = 'Error saving heights.';
    }
});
...
// Update UI to enable/disable controls based on admin status
function updateUIForAdminStatus() {
    // Get all editing controls
    const editButtons = document.querySelectorAll('button[id*="save"], button[id*="draw"], button[id*="rename"], button[id*="delete"], button[id*="edit"], button[id*="resize"]');
    const editInputs = document.querySelectorAll('input[type="text"], input[type="number"], input[type="color"], select');
    ...
    if (isAdmin) {
        ...
        editButtons.forEach(btn => {
            // Only disable editing buttons, not the sidebar toggle
            if (!btn.id.includes('sidebar-toggle')) {
                btn.disabled = true;
        ...
        // Show a message that editing is disabled
        const existingMsg = document.getElementById('readonly-message');
        if (!existingMsg) {
            const msg = document.createElement('div');
            msg.id = 'readonly-message';
            msg.style.cssText = 'position: fixed; top: 60px; right: 20px; background: #ffc107; color: #000; padding: 10px 15px; border-radius: 5px; z-index: 999; font-weight: bold; box-shadow: 0 2px 8px rgba(0,0,0,0.2);';
            msg.textContent = '📖 Read-Only Mode (View Only)';
...
canvas.addEventListener('mousedown', (e) => {
    ...
    // Use uniform scale to ensure 1cm = same pixels in all directions
    const pixelsPerCm = window.pixelsPerCm || (canvas.width / mapWidthCm);
